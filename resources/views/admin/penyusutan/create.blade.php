        ...
        <div class="max-w-3xl mx-auto px-4 sm:px-6 lg:px-8 py-4 sm:py-6">
            ...
            <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-4 sm:p-6">
                ...
                <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-4 mb-6">
                    <form method="GET" action="{{ route('admin.penyusutan.create') }}"
                    ...
                    @if ($selectedUsahaId)
                        <p class="text-slate-300 text-xs mt-2">Aset akan dicatat untuk usaha: <span
                ...
                @if (!$selectedUsahaId)
                    <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-8 text-center">
                        <p class="text-slate-400 text-sm">Pilih usaha terlebih dahulu untuk menambahkan aset</p>
                    </div>
                @else
                    <form action="{{ route('admin.penyusutan.store') }}" method="POST">
                        ...
                        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 sm:gap-6">
                            ...
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-slate-300 mb-2">Tanggal Perolehan
                            ...
                            <div>
                                ...
                                <input type="text" name="golongan"
                                    class="w-full px-3 py-2 text-xs sm:text-sm bg-slate-700/50 border border-slate-600/50 text-white placeholder-slate-400 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all"
                                    placeholder="Golongan aset" required>
                                @error('golongan')
                                    <p class="text-red-400 text-xs mt-1">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-slate-300 mb-2">Umur Ekonomis
                                    (tahun)
                                    <span class="text-red-500">*</span></label>
                            ...
                            <div>
                                <label class="block text-xs sm:text-sm font-medium text-slate-300 mb-2">Akun Beban
                                    Penyusutan
                                    <span class="text-red-500">*</span></label>

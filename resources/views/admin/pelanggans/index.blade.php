    ...
    <div class="mb-4 flex flex-col gap-3">
        <div class="flex flex-col sm:flex-row gap-3">
            <div class="flex-1 flex gap-3">
                <select id="usahaFilter" class="px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-400 focus:outline-none focus:border-blue-500 min-w-[180px]">
            ...
            <table class="w-full text-sm text-slate-200">
                <thead class="bg-slate-700 border-b border-slate-600">
                    <tr>
                        ...
                        @if(count($usahas) > 1)
                        <th class="hidden lg:table-cell px-4 py-3 text-left font-semibold text-slate-100">Usaha</th>
                        @endif
                ...
                <tbody id="tableBody">
                    @foreach ($pelanggans as $pelanggan)
                        <tr class="border-b border-slate-700 hover:bg-slate-750 transition searchable" data-search="{{ strtolower($pelanggan->nama . ' ' . $pelanggan->email . ' ' . $pelanggan->telepon) }}" data-usaha="{{ $pelanggan->usaha_id }}">
                            <td class="px-4 py-3">
                            ...
                            @if(count($usahas) > 1)
                            <td class="hidden lg:table-cell px-4 py-3 text-slate-400">
                                {{ $pelanggan->usaha->nama ?? '-' }}
                            </td>
                            @endif
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-2 flex-wrap">
                                    <button onclick="editPelanggan({{ json_encode($pelanggan) }})" class="px-3 py-1 bg-amber-600 text-white text-xs rounded hover:bg-amber-700 transition">Edit</button>
...
<dialog id="addModal" class="rounded-lg p-6 w-11/12 md:w-2/3 lg:w-1/2 bg-slate-800 border border-slate-700 shadow-xl max-h-screen overflow-y-auto">
    <form method="POST" action="{{ route('admin.pelanggans.store') }}" class="space-y-4">
        ...
        @if(count($usahas) > 1)
        <div>
            <label class="block text-sm text-slate-300 mb-1">Usaha</label>
            <select name="usaha_id" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500" required>

...
<div class="min-h-screen bg-slate-950">
    <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-6">
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-6">
            <dl class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="border-b border-slate-700/40 pb-4">
                    ...
                    <dd class="text-slate-100 text-sm">{{ \Carbon\Carbon::parse($kasbank->tanggal)->format('d M Y') }}</dd>
                </div>

                <div class="border-b border-slate-700/40 pb-4">
                    <dt class="text-xs font-semibold text-slate-400 mb-1">Total Jumlah</dt>
                    <dd class="text-blue-400 text-sm font-semibold">Rp {{ number_format($kasbank->jumlah, 0, ',', '.') }}</dd>
                </div>
                ...
                <div class="border-b border-slate-700/40 pb-4">
                    ...
                    <dd>
                        <span class="inline-flex items-center px-2 py-1 rounded text-xs font-medium {{ $kasbank->status == 'PROCESSED' ? 'bg-emerald-500/20 text-emerald-300' : 'bg-yellow-500/20 text-yellow-300' }}">
                            {{ $kasbank->status }}
                        </span>
                    </dd>
                </div>
                ...
                <div class="border-b border-slate-700/40 pb-4">
                    ...
                    <dd class="text-slate-100 text-sm">{{ $kasbank->akunPayment->kode }} - {{ $kasbank->akunPayment->name }}</dd>
                </div>

                <div class="border-b border-slate-700/40 pb-4">
                    <dt class="text-xs font-semibold text-slate-400 mb-1">Akun Lawan</dt>
                    <dd class="text-slate-100 text-sm">{{ $kasbank->akunLawan->kode }} - {{ $kasbank->akunLawan->name }}</dd>
                </div>
        ...
        <div class="mt-6 flex flex-wrap gap-3 justify-end">
            <a href="{{ route('admin.kasbank.edit', ['tipe' => $tipe, 'kasbank' => $kasbank->id]) }}" class="inline-flex items-center px-4 py-2 bg-amber-600 hover:bg-amber-700 text-white text-sm font-medium rounded-lg transition-colors duration-200">
            ...
            <form action="{{ route('admin.kasbank.destroy', ['tipe' => $tipe, 'kasbank' => $kasbank->id]) }}" method="POST" onsubmit="return confirm('Hapus transaksi ini? Jurnal akan dibalikkan.');" class="inline">
                @csrf
                @method('DELETE')

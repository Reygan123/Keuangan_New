...
<div class="p-4 sm:p-6 max-w-2xl mx-auto">
    <h1 class="text-2xl sm:text-3xl font-bold text-white mb-6">Edit Produk: {{ $product->nama }}</h1>

    <form action="{{ route('admin.products.update', $product) }}" method="POST" class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 p-5 sm:p-6 rounded-lg shadow-lg space-y-4">
        @csrf
        @method('PUT')
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            ...
            <div>
                ...
                <select name="kategori_hpp_id" id="kategori_hpp_id" required
                    ...
                    @foreach ($kategoriHpps as $kategori)
                        <option value="{{ $kategori->id }}" {{ old('kategori_hpp_id', $product->kategori_hpp_id) == $kategori->id ? 'selected' : '' }}>
                            {{ $kategori->name }}
                        </option>
                    @endforeach
                </select>
            ...
            {{-- <div>
                ...
                <input type="number" name="hpp_unit_rata2" id="hpp_unit_rata2" value="{{ old('hpp_unit_rata2', $product->hpp_unit_rata2) }}" step="any" min="0"
            ...
            <div>
                ...
                <select name="akun_pendapatan_id" id="akun_pendapatan_id" required
                    ...
                    @foreach ($akunPendapatan as $akun)
                        <option value="{{ $akun->id }}" {{ old('akun_pendapatan_id', $product->akun_pendapatan_id) == $akun->id ? 'selected' : '' }}>
                            {{ $akun->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="akun_hpp_id" id="akun_hpp_id" required
                    ...
                    @foreach ($akunHpps as $akun)
                        <option value="{{ $akun->id }}" {{ old('akun_hpp_id', $product->akun_hpp_id) == $akun->id ? 'selected' : '' }}>
                            {{ $akun->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <select name="akun_persediaan_id" id="akun_persediaan_id" required
                    ...
                    @foreach ($akunPersediaan as $akun)
                        <option value="{{ $akun->id }}" {{ old('akun_persediaan_id', $product->akun_persediaan_id) == $akun->id ? 'selected' : '' }}>
                            {{ $akun->name }}
                        </option>
                    @endforeach
                </select>
            ...
            <div>
                ...
                <input type="text" name="satuan_unit" id="satuan_unit" value="{{ old('satuan_unit', $product->satuan_unit) }}" required
            ...
            {{-- <div>
                <label for="stok" class="text-slate-300 block mb-2 text-sm font-medium">Stok</label>

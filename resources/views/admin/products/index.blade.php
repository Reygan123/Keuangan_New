...
<div class="p-4 sm:p-6 max-w-7xl mx-auto">
    <div class="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <h1 class="text-2xl sm:text-3xl font-bold text-white">Daftar Produk</h1>
        <a href="{{ route('admin.products.create') }}" class="inline-block bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg text-sm transition-colors">
            Tambah Produk
        </a>
    </div>
    ...
    <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg shadow-lg p-4 mb-6">
        <form method="GET" action="{{ route('admin.products.index') }}" class="flex flex-col sm:flex-row gap-3">
            ...
            <a href="{{ route('admin.products.index') }}" class="bg-slate-700 hover:bg-slate-600 text-white px-4 py-2 rounded-lg text-sm transition-colors">
                Reset
            </a>
        </form>
    </div>
                ...
                <tbody class="divide-y divide-slate-700/50">
                    @forelse ($products as $item)
                        <tr class="hover:bg-slate-700/30 transition-colors">
                            <td class="px-4 py-3 font-medium text-white">{{ $item->nama }}</td>

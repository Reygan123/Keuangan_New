...
<div class="min-h-screen bg-slate-950">
    <!-- Adjusted padding, reduced heading sizes for consistency with sidebar theme -->
        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="mb-6 bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-700/60 p-4">
                <div class="flex flex-col md:flex-row gap-3">
                    <!-- Added search input with filter functionality -->
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-4 md:gap-6">
                ...
                <div class="lg:col-span-2 bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-700/60 p-4 md:p-6 shadow-lg hover:border-slate-700 transition-colors duration-200">
                    <!-- Reduced font sizes and adjusted spacing for better responsive layout -->
                    <div class="space-y-4 md:space-y-5">
                        <div>
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Nama Usaha</p>
                            <p id="namaDisplay" class="text-white text-xl md:text-2xl font-bold">{{ $usaha->nama }}</p>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            ...
                            <p id="alamatDisplay" class="text-slate-200 text-sm md:text-base">{{ $usaha->alamat ?? '—' }}</p>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Email</p>
                            <p id="emailDisplay" class="text-slate-200 text-sm md:text-base break-all">{{ $usaha->email ?? '—' }}</p>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Telepon</p>
                            <p id="teleponDisplay" class="text-slate-200 text-sm md:text-base font-medium">{{ $usaha->telepon ?? '—' }}</p>
                        </div>
                        <div class="grid grid-cols-2 gap-4 border-t border-slate-700/40 pt-4">
                            <div>
                                ...
                                <p id="kodePosDisplay" class="text-slate-200 text-sm">{{ $usaha->kode_pos ?? '—' }}</p>
                            </div>
                            <div>
                                <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Kota</p>
                                <p id="kotaDisplay" class="text-slate-200 text-sm">{{ $usaha->kota ?? '—' }}</p>
                            </div>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Provinsi</p>
                            <p id="provinsiDisplay" class="text-slate-200 text-sm">{{ $usaha->provinsi ?? '—' }}</p>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">Website</p>
                            <p id="websiteDisplay" class="text-slate-200 text-sm break-all">{{ $usaha->website ?? '—' }}</p>
                        </div>
                        <div class="border-t border-slate-700/40 pt-4">
                            <p class="text-xs font-semibold text-slate-400 uppercase tracking-wide mb-1">FAQ</p>
                            <p id="faqDisplay" class="text-slate-300 text-sm leading-relaxed whitespace-pre-wrap">{{ $usaha->faq ?? '—' }}</p>
                        </div>
                    </div>
                </div>

                <!-- Action Buttons - Right Section -->
                <div class="bg-slate-800/50 backdrop-blur-sm rounded-lg border border-slate-700/60 p-4 md:p-6 shadow-lg h-fit">
                    <h3 class="text-sm md:text-base font-semibold text-white mb-4">Aksi</h3>
                    <div class="space-y-2 md:space-y-3 flex flex-col">
                        <!-- Adjusted button sizes and spacing for consistency -->
                        <button onclick="openEditModal()" class="flex items-center justify-center gap-2 px-4 py-2.5 md:py-3 bg-amber-600/90 hover:bg-amber-600 active:bg-amber-700 text-white font-semibold rounded-lg transition-colors duration-200 text-sm">
                            <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
<div id="createModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 hidden">
    <div class="bg-slate-800 rounded-lg border border-slate-700/60 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.usahas.store') }}" method="POST" class="p-4 md:p-6 grid grid-cols-2 gap-3 md:gap-4">
            @csrf
            <!-- Reduced font sizes and adjusted input spacing -->
            <div>
...
<div id="editModal" class="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center p-4 z-50 hidden">
    <div class="bg-slate-800 rounded-lg border border-slate-700/60 w-full max-w-md shadow-2xl max-h-[90vh] overflow-y-auto">
        ...
        <form action="{{ route('admin.usahas.update', $usaha) }}" method="POST" class="p-4 md:p-6 space-y-3 md:space-y-4">
            @csrf
            @method('PUT')
            <!-- Adjusted input sizes to match create modal -->
            <div>
                <label class="block text-xs md:text-sm font-semibold text-slate-200 mb-1.5">Nama Usaha <span class="text-red-400">*</span></label>
...
document.getElementById('searchInput')?.addEventListener('input', function(e) {
    ...
    const nama = document.getElementById('namaDisplay')?.textContent.toLowerCase() || '';
    const email = document.getElementById('emailDisplay')?.textContent.toLowerCase() || '';
    const telepon = document.getElementById('teleponDisplay')?.textContent.toLowerCase() || '';
    ...
    if (mainContent) {
        mainContent.style.display = isMatch || searchTerm === '' ? 'block' : 'none';
    }
});

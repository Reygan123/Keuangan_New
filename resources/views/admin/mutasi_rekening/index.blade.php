    ...
    <div class="max-w-7xl mx-auto">
        <div class="flex flex-col gap-4 md:gap-6 mb-6">
            <div class="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-4">
                ...
                @if($selectedUsahaId)
                <a href="{{ route('admin.mutasi_rekening.create', ['usaha_id' => $selectedUsahaId]) }}" class="inline-flex items-center px-3 md:px-4 py-2 bg-blue-600 hover:bg-blue-700 active:bg-blue-800 rounded-lg text-xs md:text-sm font-medium transition-colors duration-200">
                    Buat Mutasi
                </a>
                @endif
            </div>
            ...
            @if (session('error'))
            <div class="p-3 rounded-lg bg-red-900/30 border border-red-500/50 text-red-300 text-xs md:text-sm">
                {{ session('error') }}
            </div>
            @endif

            <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-3 md:p-4">
                <form method="GET" id="filterForm" class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-3">
                    <div>
                        <label class="block text-slate-400 text-xs font-semibold mb-1">Usaha</label>
                        <select name="usaha_id" onchange="document.getElementById('filterForm').submit()" class="w-full bg-slate-700/50 border border-slate-600/50 text-slate-100 rounded-lg px-3 py-2 text-xs md:text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all duration-200">
                    ...
                    <div>
                        <label class="block text-slate-400 text-xs font-semibold mb-1">Tanggal Dari</label>
                        <input type="date" name="tanggal_dari" value="{{ request('tanggal_dari') }}" onchange="document.getElementById('filterForm').submit()" class="w-full bg-slate-700/50 border border-slate-600/50 text-slate-100 rounded-lg px-3 py-2 text-xs md:text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all duration-200">
                    </div>
                    <div>
                        <label class="block text-slate-400 text-xs font-semibold mb-1">Tanggal Sampai</label>
                        <input type="date" name="tanggal_sampai" value="{{ request('tanggal_sampai') }}" onchange="document.getElementById('filterForm').submit()" class="w-full bg-slate-700/50 border border-slate-600/50 text-slate-100 rounded-lg px-3 py-2 text-xs md:text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all duration-200">
                    </div>
                    <div>
                        <label class="block text-slate-400 text-xs font-semibold mb-1">Cari Akun</label>
                        <input type="text" name="search" value="{{ request('search') }}" oninput="document.getElementById('filterForm').submit()" placeholder="Cari akun..." class="w-full bg-slate-700/50 border border-slate-600/50 text-slate-100 rounded-lg px-3 py-2 text-xs md:text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/30 transition-all duration-200">
                    </div>
                    <div class="flex gap-2 items-end">
                        <a href="{{ route('admin.mutasi_rekening.index') }}" class="w-full bg-slate-700 hover:bg-slate-600 text-slate-100 rounded-lg px-3 py-2 text-xs md:text-sm font-medium text-center transition-colors duration-200">
                            Reset
                        </a>
                    </div>
                </form>
        ...
        <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-8 text-center">
            <p class="text-slate-400 text-sm">Pilih usaha terlebih dahulu untuk melihat mutasi rekening</p>
        </div>
        @endif
    </div>
</div>

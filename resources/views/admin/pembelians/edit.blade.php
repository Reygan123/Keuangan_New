...
@section('content')
    <div class="p-4 sm:p-6 max-w-5xl mx-auto">
        ...
        <form action="{{ route('admin.pembelians.update', $pembelian->id) }}" method="POST" id="form-pembelian"
            class="space-y-4">
            @csrf
            @method('PUT')
                ...
                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3">
                    <div>
                        ...
                        <select name="label_id"
                            ...
                            @foreach ($labels as $l)
                                <option value="{{ $l->id }}" @selected($pembelian->label_id == $l->id)>{{ $l->nama_label }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    <div>
                        ...
                        <select name="supplier_id"
                            ...
                            @foreach ($suppliers as $s)
                                <option value="{{ $s->id }}" @selected($pembelian->supplier_id == $s->id)>{{ $s->nama }}
                                </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div>
                        ...
                        <select name="akun_payment_id"
                            class="w-full bg-slate-700/50 border border-slate-700 text-white px-3 py-2 rounded-lg text-sm focus:outline-none focus:border-blue-500">
                            @foreach ($akuns as $a)
                                <option value="{{ $a->id }}" @selected($pembelian->akun_payment_id == $a->id)>{{ $a->name }}
                                </option>
                            @endforeach
                        </select>
                    </div>
            ...
            <div class="bg-slate-800/50 backdrop-blur-sm border border-slate-700/60 rounded-lg p-4">
                            ...
                            @foreach ($pembelian->detailProduks as $detail)
                                <tr class="produk-row">
                                    <td class="px-3 py-2">
                                        <select name="product_id[]"
                                            ...
                                            @foreach ($products as $p)
                                                <option value="{{ $p->id }}" @selected($detail->product_id == $p->id)>
                                                    {{ $p->nama ?? $p->name }}</option>
                                            @endforeach
                                        </select>
                                    </td>
                                    <td class="px-3 py-2"><input type="number" name="kuantitas[]" step="0.01"
                                            min="0" value="{{ $detail->kuantitas }}"
                                    ...
                                    <td class="px-3 py-2"><input type="number" name="harga_satuan[]" step="0.01"
                                            min="0" value="{{ $detail->harga_satuan }}"
                ...
                <div class="mt-3 flex justify-end items-center gap-3">
                    ...
                    <span id="total-display" class="text-lg font-semibold text-white">Rp
                        {{ number_format($pembelian->jumlah, 0, ',', '.') }}</span>
                </div>
            </div>
            ...
            <div class="flex gap-3">
                <button type="submit"
                    class="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold rounded-lg text-sm">Perbarui</button>

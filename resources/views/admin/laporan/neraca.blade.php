    ...
    <div class="min-h-screen bg-gradient-to-b from-slate-900 to-slate-950 p-3 md:p-6">
        <div class="max-w-7xl mx-auto">
            <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
                ...
                <form method="GET" action="{{ route('admin.laporan.neraca') }}"
                    ...
                    @if ($usahas && $usahas->count() > 0)
                        <div>
                            ...
                            <select name="usaha_id"
                                class="bg-slate-800 border border-slate-700 text-slate-100 text-sm px-3 py-2 rounded-lg hover:border-slate-600 focus:outline-none focus:border-blue-500 transition-colors cursor-pointer">
                                @foreach ($usahas as $usahaItem)
                                    <option value="{{ $usahaItem->id }}"
                                        {{ $usahaSelected == $usahaItem->id ? 'selected' : '' }}>{{ $usahaItem->nama }}
                                    </option>
                                @endforeach
                            </select>
                        </div>
                    ...
                    <div>
                        ...
                        <select name="tahun"
                            ...
                            @for ($i = date('Y') - 5; $i <= date('Y') + 1; $i++)
                                <option value="{{ $i }}" @selected($tahun == $i)>{{ $i }}
                                </option>
                            @endfor
                        </select>
                    </div>
                    <button type="submit"
                        class="bg-blue-600 hover:bg-blue-700 text-white text-sm px-4 py-2 rounded-lg transition-colors font-semibold h-fit">Filter</button>
                </form>
            </div>
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-slate-750 rounded-lg border border-slate-700 overflow-hidden">
                                <div class="bg-slate-700 px-4 py-2 border-b border-slate-600">
                                    <h4 class="text-xs font-semibold text-slate-200 uppercase tracking-wide">Aset Lancar
                                    </h4>
                                </div>
                                <div class="divide-y divide-slate-700">
                                    @foreach ($neracaItems['ASET_LANCAR'] as $aset)
                                        <div
                                            class="px-4 py-2 flex justify-between hover:bg-slate-700 transition-colors text-xs">
                                            <span class="text-slate-300">{{ $aset['name'] }}</span>
                                            <span
                                                class="text-slate-100 font-medium">{{ number_format($aset['saldo_akhir'], 0, ',', '.') }}</span>
                                        </div>
                                    @endforeach
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-slate-100">{{ number_format($totalAsetLancar, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-750 rounded-lg border border-slate-700 overflow-hidden">
                                <div class="bg-slate-700 px-4 py-2 border-b border-slate-600">
                                    <h4 class="text-xs font-semibold text-slate-200 uppercase tracking-wide">Aset Tetap
                                        (Bruto)</h4>
                                </div>
                                <div class="divide-y divide-slate-700">
                                    ...
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-slate-100">{{ number_format($totalAsetTetapBruto, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-750 rounded-lg border border-slate-700 overflow-hidden">
                                <div class="bg-slate-700 px-4 py-2 border-b border-slate-600">
                                    <h4 class="text-xs font-semibold text-slate-200 uppercase tracking-wide">Akumulasi
                                        Penyusutan</h4>
                                </div>
                                <div class="divide-y divide-slate-700">
                                    @foreach ($neracaItems['AKUMULASI_PENYUSUTAN'] as $akumulasi)
                                        <div
                                            ...
                                            <span
                                                class="text-red-400 font-medium">({{ number_format(abs($akumulasi['saldo_akhir']), 0, ',', '.') }})</span>
                                        </div>
                                    @endforeach
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-red-400">({{ number_format(abs($totalAkumulasiPenyusutan), 0, ',', '.') }})</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-blue-950 border border-blue-800 rounded-lg px-4 py-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-bold text-blue-300">{{ number_format($totalAsetTetapNeto, 0, ',', '.') }}</span>
                                </div>
                            </div>
                            ...
                            <div class="bg-green-950 border border-green-800 rounded-lg px-4 py-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-bold text-green-300">{{ number_format($totalAset, 0, ',', '.') }}</span>
                                </div>
                            </div>
                        </div>

                        <div class="space-y-4">
                            <h3 class="text-sm font-bold text-slate-100 bg-slate-700 px-4 py-2 rounded-lg">KEWAJIBAN &
                                EKUITAS</h3>

                            <div class="bg-slate-750 rounded-lg border border-slate-700 overflow-hidden">
                                <div class="bg-slate-700 px-4 py-2 border-b border-slate-600">
                                    <h4 class="text-xs font-semibold text-slate-200 uppercase tracking-wide">Kewajiban
                                        Lancar</h4>
                                </div>
                                <div class="divide-y divide-slate-700">
                                    @foreach ($neracaItems['KEWAJIBAN_LANCAR'] as $kewajiban)
                                        <div
                                            ...
                                            <span
                                                class="text-slate-100 font-medium">{{ number_format($kewajiban['saldo_akhir'], 0, ',', '.') }}</span>
                                        </div>
                                    @endforeach
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-slate-100">{{ number_format($totalKewajibanLancar, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-slate-750 rounded-lg border border-slate-700 overflow-hidden">
                                ...
                                <div class="divide-y divide-slate-700">
                                    ...
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-slate-100">{{ number_format($totalKewajibanPanjang, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-orange-950 border border-orange-800 rounded-lg px-4 py-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-bold text-orange-300">{{ number_format($totalKewajiban, 0, ',', '.') }}</span>
                                </div>
                            </div>
                                ...
                                <div class="divide-y divide-slate-700">
                                    @foreach ($neracaItems['EKUITAS'] as $ekuitas)
                                        <div
                                            ...
                                            <span
                                                class="text-slate-100 font-medium">{{ number_format($ekuitas['saldo_akhir'], 0, ',', '.') }}</span>
                                        </div>
                                    @endforeach
                                    <div
                                        class="px-4 py-2 flex justify-between hover:bg-slate-700 transition-colors text-xs">
                                        ...
                                        <span
                                            class="text-slate-100 font-medium">{{ number_format($labaBersih, 0, ',', '.') }}</span>
                                    </div>
                                    <div class="px-4 py-2 flex justify-between bg-slate-700 text-xs font-semibold">
                                        <span class="text-slate-200">Total</span>
                                        <span
                                            class="text-slate-100">{{ number_format($totalEkuitas, 0, ',', '.') }}</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div class="bg-purple-950 border border-purple-800 rounded-lg px-4 py-3">
                                <div class="flex justify-between items-center">
                                    ...
                                    <span
                                        class="text-sm font-bold text-purple-300">{{ number_format($totalKewajibanEkuitas, 0, ',', '.') }}</span>
                                </div>
                            </div>

                            <div
                                class="bg-slate-750 rounded-lg border {{ $statusSeimbang == 'Seimbang' ? 'border-green-600' : 'border-red-600' }} overflow-hidden">
                                <div class="px-4 py-3 flex justify-between items-center">
                                    <span class="text-xs font-semibold text-slate-200">Status Neraca</span>
                                    <span
                                        class="text-sm font-bold {{ $statusSeimbang == 'Seimbang' ? 'text-green-400' : 'text-red-400' }}">{{ $statusSeimbang }}</span>
                                </div>
                                @if ($statusSeimbang == 'Tidak Seimbang')
                                    <div
                                        class="px-4 py-2 bg-slate-700 flex justify-between items-center border-t border-red-600 text-xs">
                                        <span class="text-slate-300">Selisih</span>
                                        <span
                                            class="text-red-400 font-semibold">{{ number_format($selisih, 0, ',', '.') }}</span>
                                    </div>
                                @endif
                            </div>
                        </div>

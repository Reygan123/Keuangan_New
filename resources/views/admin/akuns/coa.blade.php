    ...
    <div class="max-w-7xl mx-auto">
        <div class="mb-8">
            ...
            <p class="text-slate-400 text-sm">
                ...
                <a href="{{ route('admin.akuns.index') }}" class="text-blue-400 hover:text-blue-300 transition">Manajemen Akun</a>.
            </p>
        </div>
                ...
                <div id="neracaContainer" class="space-y-4">
                    @foreach ($neracaGroups as $klasifikasi => $kelompokGroups)
                        <div class="neracaItem">
                            <h3 class="text-sm font-semibold px-3 py-2 bg-slate-800 text-blue-400 rounded-t-lg border-l-3 border-blue-500">
                                {{ $klasifikasi }}
                            </h3>
                            ...
                            @foreach ($kelompokGroups as $namaKelompok => $akuns)
                                @if(count($akuns) > 0)
                                    <div class="bg-slate-850 border border-t-0 border-slate-700">
                                        <h4 class="text-xs font-semibold px-3 py-2 bg-slate-800 text-slate-300 uppercase tracking-wide border-b border-slate-700">
                                            {{ $namaKelompok }}
                                        </h4>

                                        <div class="divide-y divide-slate-700">
                                            @foreach ($akuns as $akun)
                                                <div class="px-3 py-2 text-xs flex justify-between items-center hover:bg-slate-800 transition duration-150 akunRow" data-akun="{{ strtolower($akun->name) }}">
                                                    <span class="text-slate-400 font-mono w-16 flex-shrink-0">{{ $akun->id }}</span>
                ...
                <div id="labaRugiContainer" class="space-y-4">
                    @foreach ($labaRugiGroups as $klasifikasi => $kelompokGroups)
                        <div class="labaRugiItem">
                            <h3 class="text-sm font-semibold px-3 py-2 bg-slate-800 text-amber-400 rounded-t-lg border-l-3 border-amber-500">
                                {{ $klasifikasi }}
                            </h3>
        ...
        <div class="mt-8 p-4 bg-slate-800 border border-slate-700 rounded-lg">
            ...
            <ul class="text-xs text-slate-400 space-y-1">
                ...
                <li><span class="text-slate-300">Pengelompokan:</span> Aset/Kewajiban berdasarkan Sub Klasifikasi; yang lain menggunakan Nama Kelompok</li>
            </ul>
        </div>
    </div>
</div>
...
<script>
    ...
    function filterAkun(searchInput, container) {
        ...
        sections.forEach(section => {
            const hasVisible = section.querySelector('.akunRow[style*="display: flex"], .akunRow:not([style*="display: none"])');
            section.style.display = visibleGroups.has(section) || (query === '' && !section.classList.contains('bg-slate-850')) ? '' : 'none';
        });
    }

...
<div class="p-4 sm:p-6">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        ...
        @if($selectedUsahaId)
        <a href="{{ route('admin.penjualans.create', ['usaha_id' => $selectedUsahaId]) }}" class="inline-flex items-center gap-2 bg-blue-600 hover:bg-blue-700 text-white font-semibold px-4 py-2 rounded-lg shadow text-sm">
            Tambah Penjualan
        </a>
        @endif
    </div>
    ...
    <div class="bg-slate-800/40 backdrop-blur-sm rounded-lg border border-slate-700/60 p-4 mb-4">
        <form method="GET" id="filterForm" class="grid grid-cols-1 sm:grid-cols-4 gap-3">
            ...
            <div>
                <label class="block text-slate-400 text-xs font-semibold mb-1">Cari Pelanggan</label>
                <input type="text" name="search" placeholder="Cari pelanggan..." value="{{ request('search') }}" oninput="document.getElementById('filterForm').submit()" class="w-full bg-slate-700/50 border border-slate-700 text-white px-3 py-2 rounded-lg text-sm placeholder-slate-400 focus:outline-none focus:border-blue-500">
            </div>
            <div>
            ...
            <div class="flex gap-2 items-end">
                ...
                <a href="{{ route('admin.penjualans.index') }}" class="px-4 py-2 bg-slate-700 hover:bg-slate-600 text-white rounded-lg text-sm h-10 flex items-center">Reset</a>
            </div>
        </form>
    </div>
    ...
    <div class="bg-slate-800/40 backdrop-blur-sm rounded-lg border border-slate-700/60 p-8 text-center">
        <p class="text-slate-400 text-sm">Pilih usaha terlebih dahulu untuk melihat transaksi penjualan</p>
    </div>
    @endif
</div>

...
<div class="p-4 md:p-6 lg:p-8">
    <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4 mb-6">
        <h1 class="text-xl md:text-2xl font-bold text-slate-200">Daftar Users</h1>
    ...
    @if(session('error'))
        <div class="mb-4 p-3 bg-red-900 text-red-100 rounded text-sm border border-red-700">{{ session('error') }}</div>
    @endif
    ...
    <div class="mb-4 flex flex-col gap-3">
        <div class="flex flex-col sm:flex-row gap-3">
            ...
            <div class="flex gap-2">
                <button onclick="applyFilter()" class="px-4 py-2 bg-blue-600 text-white text-sm rounded hover:bg-blue-700 transition whitespace-nowrap">Filter</button>
            ...
            <table class="w-full text-sm text-slate-200">
                <thead class="bg-slate-700 border-b border-slate-600">
                    <tr>
                        ...
                        <th class="hidden md:table-cell px-4 py-3 text-left font-semibold text-slate-100">Role</th>
                        <th class="hidden lg:table-cell px-4 py-3 text-left font-semibold text-slate-100">Password</th>
                        <th class="px-4 py-3 text-center font-semibold text-slate-100">Aksi</th>
                    </tr>
                </thead>
                <tbody id="tableBody">
                    @foreach ($users as $user)
                        <tr class="border-b border-slate-700 hover:bg-slate-750 transition searchable" data-search="{{ strtolower($user->nama . ' ' . $user->email . ' ' . $user->role . ' ' . $user->password) }}">
                            <td class="px-4 py-3">
                                <div class="font-medium text-slate-100">{{ $user->name }}</div>
                            </td>
                            <td class="px-4 py-3">
                                <div class="text-slate-300 truncate">{{ $user->email }}</div>
                            </td>
                            <td class="hidden md:table-cell px-4 py-3 text-slate-300">{{ $user->role }}</td>
                            <td class="hidden lg:table-cell px-4 py-3 text-slate-400 truncate max-w-xs">{{ $user->password }}</td>
                            <td class="px-4 py-3">
                                <div class="flex justify-center gap-2 flex-wrap">
                                    <button onclick="editUser({{ json_encode($user) }})" class="px-3 py-1 bg-amber-600 text-white text-xs rounded hover:bg-amber-700 transition">Edit</button>
                                    <form action="{{ route('super_admin.users.destroy', $user->id) }}" method="POST" onsubmit="return confirm('Yakin ingin menghapus?')" class="inline">
                                        @csrf
                                        @method('DELETE')
...
<dialog id="addModal" class="rounded-lg p-6 w-11/12 md:w-2/3 lg:w-1/2 bg-slate-800 border border-slate-700 shadow-xl max-h-screen overflow-y-auto">
    <form method="POST" action="{{ route('super_admin.users.store') }}" class="space-y-4">
        @csrf
        <h2 class="text-lg md:text-xl font-semibold text-slate-100 mb-4">Tambah User</h2>

        <div>
            <label class="block text-sm text-slate-300 mb-1">Nama</label>
            <input name="name" placeholder="Nama" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500" required>
        </div>
        ...
        <div>
            ...
            <select name="role" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500" required>
        ...
        <div>
            ...
            <input name="password" placeholder="Password" type="password" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500">
        </div>
...
<dialog id="editModal" class="rounded-lg p-6 w-11/12 md:w-2/3 lg:w-1/2 bg-slate-800 border border-slate-700 shadow-xl max-h-screen overflow-y-auto">
    <form method="POST" id="editForm" class="space-y-4">
        @csrf
        @method('PUT')
        <h2 class="text-lg md:text-xl font-semibold text-slate-100 mb-4">Edit User</h2>

        <div>
            <label class="block text-sm text-slate-300 mb-1">Nama</label>
            <input name="name" id="editNama" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500" required>
        </div>
        ...
        <div>
            <label class="block text-sm text-slate-300 mb-1">Role</label>
            <select name="role" id="editRole" class="w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500" required>
                <option value="">Pilih Role</option>
                <option value="super_admin">Super Admin</option>
                <option value="admin">Admin</option>
                <option value="koperasi">Koperasi</option>
        ...
        <div>
            {{-- <label class="block text-sm text-slate-300 mb-1">Password</label> --}}
            <input name="password" id="editPassword" type="password" class="hidden w-full px-3 py-2 text-sm bg-slate-700 text-slate-100 border border-slate-600 rounded placeholder-slate-500 focus:outline-none focus:border-blue-500">
        </div>
...
function editUser(data) {
    ...
    form.action = `/super_admin/users/${data.id}`
...
function applyFilter() {
    ...
    rows.forEach(row => {
        const searchMatch = row.dataset.search.includes(searchValue)
        if (searchMatch) {
            row.style.display = ''
            visibleCount++
        } else {
